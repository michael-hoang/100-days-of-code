...
def get_upgrade_prices() -> list:
    """Get updated prices for available upgrades from Store."""
    prices = []
    unlocked_ugrades = driver.find_elements(By.CSS_SELECTOR, '.product.unlocked.enabled')
    for element in unlocked_ugrades:
        price = int(element.find_element(By.CSS_SELECTOR, 'span .price').text)
        prices.append(price)

    return prices

def upgrade():
    """Perform action chains on cursor to purchase upgrades from Store."""
    upgrade_prices = get_upgrade_prices()
    most_expensive = upgrade_prices.index(max(upgrade_prices))
